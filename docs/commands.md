...
```
docker exec -it payment_gateway_backend bash
```
...
```
docker exec -it payment_gateway_frontend sh
```
...
```
docker exec -it payment_gateway_mysql mysql -u root -p
```

### Backup database
```
docker exec payment_gateway_mysql mysqldump -u root -p payment_gateway > backup.sql
```

### Restore database
```
cat backup.sql | docker exec -i payment_gateway_mysql mysql -u root -p payment_gateway
```
...
### Enter Redis CLI
```
docker exec -it payment_gateway_redis redis-cli
```
