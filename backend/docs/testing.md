...
## Safety model
- Tests are pinned to `APP_ENV=testing` and testing DB (`payment_gateway_testing`) by:
  - `phpunit.xml` env values
  - `tests/TestCase.php` bootstrap env enforcement
  - `TestingDatabaseGuard` fail-fast protection
- Do not run tests in parallel against the same shared `payment_gateway_testing` database.
